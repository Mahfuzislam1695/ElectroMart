...
export default function SearchFilters({ query }: SearchFiltersProps) {
  ...
  const getFiltersForQuery = (searchQuery: string) => {
    if (searchQuery.toLowerCase().includes("ac") || searchQuery.toLowerCase().includes("air conditioner")) {
      return {
        brand: ["LG", "Samsung", "Daikin", "Voltas", "Hitachi", "Blue Star"],
        tonnage: ["1 Ton", "1.5 Ton", "2 Ton", "2.5 Ton"],
        starRating: ["3 Star", "4 Star", "5 Star"],
        type: ["Split AC", "Window AC", "Inverter AC", "Non-Inverter"],
      }
    } else if (searchQuery.toLowerCase().includes("refrigerator") || searchQuery.toLowerCase().includes("fridge")) {
      return {
        brand: ["LG", "Samsung", "Whirlpool", "Godrej", "Haier"],
    ...
    return {
      brand: ["LG", "Samsung", "Sony", "Panasonic", "Philips"],
      category: ["Electronics", "Home Appliances", "Mobile", "TV", "Audio"],
    }
  }
  ...
  return (
    <div className="glass-card p-6 rounded-2xl sticky top-24">
      ...
      <div className="mb-6">
        <label className="flex items-center space-x-2 cursor-pointer">
          <Checkbox
            checked={selectedFilters.availability?.includes("in-stock") || false}
            onCheckedChange={(checked) => updateFilters("availability", "in-stock", checked as boolean)}
          />
